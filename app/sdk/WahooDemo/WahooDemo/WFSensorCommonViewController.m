...
- (void)requestConnectionToDevice:(WFDeviceParams*)devParams
{
    ...
    NSError* error = nil;
    self.sensorConnection = [hardwareConnector requestSensorConnection:params withProximity:WF_PROXIMITY_RANGE_DISABLED error:&error];

    // When ANT+ discovery is finished the Wahoo Key is reset, this sometimes means that the HW isn't ready when the connection is requested
    // Directly after the discovery process, you might want to handle it in a little more detail in your app, but here we just retry.
    if(error && error.code == WFConnectionErrorHardwareNotReady) {
        
        [connectButton setTitle:@"Please Wait..." forState:UIControlStateNormal];
